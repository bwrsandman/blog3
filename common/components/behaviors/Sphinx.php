...
namespace nizsheanez\yii\behaviors;
...
class RTSphinx extends Behavior
{
    ...
    public function deleteIndex($ids = null)
    {
        ...
        if ($ids === null) {
            $ids = array($this->owner->getPrimaryKey());
        } elseif (!is_array($ids)) {
            throw new Exception('The param "ids" should be array.');
        }
    ...
    /**
     * Responds to {@link Model::onAfterSave} event.
...
class SphinxCommand extends \yii\db\Command
{
    ...
    protected function _intoCommand($table, $columns, $type = self::INSERT_COMMAND)
    {
        ...
        foreach ($columns as $name => $value) {
            $names[] = $name === 'id' ? $name : $this->db->quoteColumnName($name);
        ...
        $sql = $type . ' INTO ' . $this->db->quoteTableName($table)
