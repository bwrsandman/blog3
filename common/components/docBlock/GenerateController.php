...
class GenerateController extends Controller
{
    ...
    {
        //configuring
        $config = require(__DIR__ . '/configs/stdConfig.php');
        Yii::configure($this, $config);
        if ($this->config) { //apply additional config if set
            Yii::configure($this, is_array($this->config) ? $this->config : require(Yii::getAlias($this->config)));
        }

        //set translator
        Yii::$app->setComponent('docBlockMessage', Yii::createObject($this->messageSource));
